...
void InitPersons() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    pos->set_y((qSin(pos->x() * r1 * 5) * qSin(qCos(pos->x() * r2) * r4 * 3) + r3 * qCos(qSin(pos->x()) * r4 * 2)) *
                   yScale +
               yOffset);
    // vel
...
QVariant ReflectVaule(const google::protobuf::Message &message,
...
google::protobuf::Message const &GetHeader(google::protobuf::Message const &msg, QString const &name = "header") {
  auto *descriptor = msg.GetDescriptor();
  auto *reflection = msg.GetReflection();
  auto *single_filed = descriptor->FindFieldByName(name.toStdString());
  auto const &result = reflection->GetMessage(msg, single_filed);

  return result;
}

double GetHeaderT0(google::protobuf::Message const &msg, QString const &name = "header") {
  auto *descriptor = msg.GetDescriptor();
  auto *reflection = msg.GetReflection();
  auto *single_filed = descriptor->FindFieldByName(name.toStdString());
  auto const &result = reflection->GetMessage(msg, single_filed);
  ...
  reflection = result.GetReflection();
  single_filed = descriptor->FindFieldByName("t0");

  auto const &t0 = ReflectVaule(result, descriptor, reflection, single_filed);
...
QVariant GetValueByHeaderName(const google::protobuf::Message &msg, QString const &name) {
  ...
  for (int i = 0; i < parts.size(); ++i) {
    ...
    if (!single_filed) { return QVariant(); }
    // 如果已经到底
    if (i == parts.size() - 1) {
