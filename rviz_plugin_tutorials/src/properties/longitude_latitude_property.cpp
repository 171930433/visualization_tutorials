...
namespace rviz
{
  ...
  LatLonProperty::LatLonProperty(const QString &name,
                                 ...
                                 QObject *receiver)
      : Property(name, QVariant(), description, parent, changed_slot, receiver), vector_(default_value), ignore_child_updates_(false)
  {
    // 单位
  ...
  void LatLonProperty::updateFromChildren()
  {
    ...
    {
      vector_with_units_ = Eigen::Vector3d{x_->getValue().toDouble(), y_->getValue().toDouble(), z_->getValue().toDouble()};
  ...
  {
    value_ =
        QString("%1; %2; %3").arg(vector_with_units_.x(), 0, 'f', 8).arg(vector_with_units_.y(), 0, 'f', 8).arg(vector_with_units_.z(), 0, 'f', 3);
  }
