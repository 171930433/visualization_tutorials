...
void MultiMatrixDisplay::UpdateChannelCount()
{
  ...
  else
  {
    for (int i = old_count; i < new_count; ++i)
    {
      ...
      auto single_changel = std::make_shared<rviz::EnumProperty>(channel_header, "", "data_channel", counts_prop_, SLOT(UpdateChannel()));
      connect(single_changel.get(), &rviz::EnumProperty::requestOptions, this, &MultiMatrixDisplay::ListCurrentChannel);
      data_channels_.push_back(single_changel);
    }
  }
}

void MultiMatrixDisplay::ListCurrentChannel(rviz::EnumProperty *topics)
{
