...
TrajectoryDisplay::TrajectoryDisplay() {
  ...
  focus_when_select_ = new rviz_common::properties::BoolProperty(
      "foucs when select", true, "focus the selected points", this, SLOT(UpdateFocusWhenSelect()));
  counts_prop_ =
      new rviz_common::properties::IntProperty("trajectory counts", 0, "the number of trajectory counts", this, SLOT(UpdateGraphCount()));
...
void TrajectoryDisplay::UpdateGraphCount() {
  ...
  auto when_deleted = [this](SubCurveProperty *graph) {
  ...
  for (int i = 0; i < new_count; ++i) {
    ...
    auto *curve = new SubCurveProperty(header, "", "", this);
    connect(curve, &Property::changed, this, &TrajectoryDisplay::UpdateTopic);
    graphs_[i] = std::shared_ptr<SubCurveProperty>(curve, when_deleted);
  }
...
void TrajectoryDisplay::UpdateTopic() {
  ...
  auto *curve_prop = dynamic_cast<SubCurveProperty *>(sender());
...
void TrajectoryDisplay::save(rviz_common::Config config) const { rviz_common::Display::save(config); }

#include <pluginlib/class_list_macros.hpp>
PLUGINLIB_EXPORT_CLASS(TrajectoryDisplay, rviz_common::Display)